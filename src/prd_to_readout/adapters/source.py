...
class FileSource:
    ...
    def load(self, runner: DuckDBRunner) -> int:
        ...
        missing = [c for c in CANONICAL if self._col(c) not in cols]
        if missing:
            need = ", ".join(self._col(c) for c in missing)
            raise ValueError(
                f"Events file is missing required column(s): {need}. "
                f"Found columns: {', '.join(cols)}. Provide a column mapping if names differ."
            )

        props_col = self._col("props") if self._col("props") in cols else ("props" if "props" in cols else None)
        skip = {self._col(c) for c in CANONICAL} | ({props_col} if props_col else set())

        events = []
        for row in rows:
            rec = dict(zip(cols, row, strict=False))
            props = self._extract_props(rec, props_col, skip)
            events.append({
                "event_name": rec[self._col("event_name")],
                "user_id": str(rec[self._col("user_id")]),
                "arm": str(rec[self._col("arm")]),
                "ts": rec[self._col("ts")],
                "props": props,
            })
        runner.load_raw_events(events)
        ...
        if props_col is not None:
            ...
            if isinstance(val, str) and val.strip():
                try:
                    ...
                    return {"raw": val}
            return {}
        # No props column: pack any leftover columns into props.
