...
def _gate_artifact(cfg: Config, stage: str) -> Path | None:
    return {
        "metric": cfg.paths.blueprint_doc,
        "logging": cfg.paths.spec_doc,
        "logging_qa": cfg.paths.qa_report,
        "query": cfg.paths.models_dir / "metrics_daily.sql",
    }.get(stage)
...
def _load_blueprint(cfg: Config):
    ...
    if not cfg.paths.blueprint.exists():
        raise GateError("No metric_plan.yaml yet. Run 'metric <prd>' first.")
...
def _load_tracking(cfg: Config):
    ...
    if not cfg.paths.tracking_schema.exists():
        raise GateError("No tracking_schema.json yet. Run 'logging' first.")
...
# --------------------------------------------------------------------------- #
def _do_metric(cfg: Config, llm, prd_text: str):
...
def _do_query(cfg: Config, llm, blueprint, tracking):
    ...
    if not runner.table_exists("raw_events"):
        runner.close()
        raise GateError("No events ingested yet. Run 'verify-logging' first.")
...
def _evaluate(cfg: Config, blueprint, tracking, state: WorkflowState):
    ...
    if not runner.table_exists("metrics_daily"):
        runner.close()
        raise GateError("No metrics_daily table yet. Run 'query' first.")
...
def status(workdir: Path = typer.Option(Path.cwd(), "--workdir", "-w")):
    ...
    if not cfg.paths.state.exists():
        console.print("[yellow]No workflow yet.[/] Run 'prd-to-readout init' or 'metric <prd>'.")
        raise typer.Exit(0)
    state = _state(cfg)
...
def metric(
    ...
    """Stage 1: PRD -> metric_plan.yaml (then review & approve)."""
...
def logging_stage(
    ...
    """Stage 2: metric plan -> tracking spec + snippets, handed off to an engineer."""
    cfg = _config(workdir, model, None)
    state = _state(cfg)
    _advance(cfg, state, "logging")
    _require_model(cfg)
    blueprint = _load_blueprint(cfg)
    tracking = _do_logging(cfg, _llm(cfg), blueprint)
    ...
    _finish_stage(cfg, state, "logging", [cfg.paths.spec_doc, cfg.paths.tracking_schema], yes=yes)
    _hint(state)


# --------------------------------------------------------------------------- #
# stage 3: verify-logging  (Phase 4 makes the checks real)
# --------------------------------------------------------------------------- #
@app.command(name="verify-logging")
@handle_errors
def verify_logging(
    ...
    from .core import logging_qa

    cfg = _config(workdir, None, None)
    state = _state(cfg)
    _advance(cfg, state, "logging_qa")
    blueprint = _load_blueprint(cfg)
    ...
    cfg.paths.qa_report.write_text(logging_qa.render_qa_report(report, blueprint))
    runner.close()
    ...
    if not report.passed and not force:
        workflow.request_changes(state, "logging_qa",
                                 note="QA failed; fix logging or re-run with --force")
        _save(state, cfg)
        console.print(f"[bold red]Logging QA failed.[/] See {cfg.paths.qa_report}")
        raise typer.Exit(1)
    ...
    _finish_stage(cfg, state, "logging_qa", [cfg.paths.qa_report, cfg.paths.db], yes=yes)
    _hint(state)
...
def query(
    ...
    cfg = _config(workdir, model, None)
    state = _state(cfg)
    _advance(cfg, state, "query")
    _require_model(cfg)
    blueprint = _load_blueprint(cfg)
    tracking = _load_tracking(cfg)
    sql, attempts = _do_query(cfg, _llm(cfg), blueprint, tracking)
    console.print(f"  SQL passed after [cyan]{attempts}[/] attempt(s) → models/metrics_daily.sql")
    _finish_stage(cfg, state, "query", [cfg.paths.models_dir / "metrics_daily.sql"], yes=yes)
    _hint(state)
...
def _window_status(state: WorkflowState, window_days: int | None) -> tuple[bool, str]:
    ...
    if not state.launch_date:
        return False, "launch date unknown (run verify-logging first)"
...
def pulse(workdir: Path = typer.Option(Path.cwd(), "--workdir", "-w")):
    ...
    cfg = _config(workdir, None, None)
    state = _state(cfg)
    if state.stage("query").status not in ("approved", "done"):
        raise GateError("Approve 'query' before monitoring with pulse.")
    blueprint = _load_blueprint(cfg)
    tracking = _load_tracking(cfg)
...
def _resolve_stage(name: str) -> str:
    if name in STAGE_ORDER:  # exact match wins (e.g. 'logging' vs 'logging_qa')
        return name
...
def run(
    ...
    steps = [
        ("metric", [cfg.paths.blueprint_doc, cfg.paths.blueprint]),
        ("logging", [cfg.paths.spec_doc, cfg.paths.tracking_schema]),
        ("logging_qa", [cfg.paths.db]),
        ("query", [cfg.paths.models_dir / "metrics_daily.sql"]),
    ...
    for stage, artifacts in steps:
        ...
        if stage == "metric":
            ...
            qa = logging_qa.run_qa(runner, bp, tr)
            cfg.paths.qa_report.write_text(logging_qa.render_qa_report(qa, bp))
            runner.close()
            console.print(f"  ingested [cyan]{n}[/] events; QA {'passed' if qa.passed else 'FAILED'}")
            if not qa.passed:
                workflow.request_changes(state, "logging_qa", note="QA failed")
                _save(state, cfg)
                console.print(f"[bold red]Paused:[/] logging QA failed. See {cfg.paths.qa_report}")
                _hint(state)
                return
            if state.launch_date is None:
                state.launch_date = date.today().isoformat()
        elif stage == "query":
            _, attempts = _do_query(cfg, llm, _load_blueprint(cfg), _load_tracking(cfg))
