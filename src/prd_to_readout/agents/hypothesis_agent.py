...
def render_blueprint_doc(bp: AnalyticsBlueprint) -> str:
    ...
    h = bp.hypotheses
    lines = [
        f"# Analytics Blueprint: {bp.feature_name}",
        "",
        f"> {bp.summary}",
        "",
        "_This is the plan we will measure against. If anything looks off, edit_ "
        "`analytics_blueprint.yaml` _(the machine-readable file next to this one) and re-run_ "
        "`prd-to-readout hypothesize`.",
        "",
        "## The bet",
        "",
    ...
    lines += ["---", "*Generated by prd-to-readout from your PRD. "
              "Source of truth: analytics_blueprint.yaml.*", ""]
    return "\n".join(lines)
